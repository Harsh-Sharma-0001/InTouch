...
app.use(
  "/uploads/recordings",
  express.static(path.resolve("uploads/recordings"))
);
...
io.on("connection", (socket) => {
  ...
  socket.on("join-room", ({ roomId, user }) => {
    ...
    if (!user) {
      console.log(
        `User without data (${socket.id}) tried to join room ${roomId}`
      );
      return;
    }
    ...
    console.log(
      `User ${userWithRole.name || "Unknown"} (${userWithRole.role}) (${
        socket.id
      }) joined room ${roomId}`
    );
    console.log(
  ...
  socket.on("leave-room", (roomId) => {
    if (rooms[roomId]) {
      ...
      if (userIndex !== -1) {
        ...
        console.log(
          `User ${user.name || "Unknown"} (${socket.id}) left room ${roomId}`
        );
      }
    }
    socket.leave(roomId);
  ...
  const leaveRoom = () => {
    ...
    if (roomIdToRemove) {
      ...
      console.log(
        `User ${userToRemove?.name} (${socket.id}) left room ${roomIdToRemove}`
      );
...
server.listen(PORT, "0.0.0.0", () => {
  ...
  console.log(
    `🌐 CORS allowed origins: ${process.env.FRONTEND_URL || "localhost"}`
  );
});
